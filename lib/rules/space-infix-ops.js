...
module.exports = function(context) {
    ...
    function report(mainNode, culpritToken) {
        context.report({
            ...
            fix: function(fixer) {
                var previousToken = context.getTokensBefore(culpritToken, 1);
                var afterToken = context.getTokensAfter(culpritToken, 1);
                var fixString = "";

                if (culpritToken.range[0] - previousToken[0].range[1] === 0) {
                    fixString = " ";
                }

                fixString += culpritToken.value;

                if (afterToken[0].range[0] - culpritToken.range[1] === 0) {
                    fixString += " ";
                }

                return fixer.replaceTextRange(culpritToken.range, fixString);
            }
        });
    }
